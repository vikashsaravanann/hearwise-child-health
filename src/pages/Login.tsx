...
export default function Login() {
  ...
  const WaveformBars = () => (
    <div className="flex items-end gap-0.5 h-8">
      {[...Array(20)].map((_, i) => (
        <motion.div
          key={i}
          className="w-1 rounded-full bg-red-500/40"
  ...
  const GridBg = () => (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      />
      <motion.div
        className="absolute top-1/4 left-1/4 w-80 h-80 rounded-full bg-red-500/5 blur-[100px]"
        animate={{ scale: [1, 1.2, 1], opacity: [0.3, 0.5, 0.3] }}
        transition={{ duration: 6, repeat: Infinity }}
      />
      <motion.div
        className="absolute bottom-1/4 right-1/4 w-60 h-60 rounded-full bg-rose-500/5 blur-[80px]"
        ...
        <div className="relative rounded-3xl border border-[#1a1a1a] bg-[#0a0a0a] p-8 shadow-2xl">
          ...
          <div className="text-center mb-8">
            <motion.div
              ...
              className="inline-flex items-center justify-center w-20 h-20 rounded-full overflow-hidden shadow-xl shadow-red-500/20 border border-red-500/20 mb-4 bg-white/5"
            >
            ...
            >
              ...
              {tab === 'google' && (
                <div className="space-y-4">
                  ...
                  <motion.button
                    ...
                    disabled={loading}
                    className="w-full py-4 rounded-xl bg-red-500 text-white font-black text-sm flex items-center justify-center gap-2 shadow-lg shadow-red-500/20 hover:bg-red-400 transition-all disabled:opacity-50"
                  >
                    {loading ? (
              ...
              {tab === 'email' && step === 'input' && (
                <div className="space-y-4">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-4 rounded-xl bg-[#111111] border border-[#222222] text-white text-sm placeholder-gray-600 focus:outline-none focus:border-red-500 transition-colors"
                  />
                  <motion.button
                    ...
                    disabled={loading}
                    className="w-full py-4 rounded-xl bg-red-500 text-white font-black text-sm uppercase tracking-widest hover:bg-red-400 transition-colors disabled:opacity-50 shadow-lg shadow-red-500/30"
                  >
                    {loading ? (
              ...
              {tab === 'email' && step === 'otp' && (
                <div className="space-y-4">
                  ...
                  <p className="text-red-400 text-sm text-center font-black uppercase tracking-wider mb-6">
                    {email}
                  </p>
                  <div className="flex gap-2 justify-center">
                    {otp.map((val, idx) => (
                      <input
                        ...
                        className="w-11 h-14 rounded-xl bg-[#111111] border border-[#222222] text-white text-xl font-black text-center focus:outline-none focus:border-red-500 transition-colors"
                      />
                    ))}
                  </div>
                  <motion.button
